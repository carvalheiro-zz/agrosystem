	...
	<div class="panel-heading" style="font-size: 20px; padding: 0px 15px">
		<div class="row">
			...
			<div class="col-xs-12 col-sm-10 col-md-10 col-lg-10 text-right">
				<label>Total NF R$&nbsp;${notaFiscalRateioReceitaForm.notaFiscalRateio.valorTotal}</label>
			</div>
		</div>
			...
			<div class="form-group col-xs-12 col-sm-12 col-md-12 col-lg-12">
				...
				<div class="table-responsive">
					<table class="table table-bordered table-striped table-hover" style="font-size: 11px;">
						...
						<tbody>
							<logic:iterate indexId="i" id="itemCorrente" name="notaFiscalRateioReceitaForm" property="notaFiscalRateio.itens" type="br.com.srcsoftware.sistema.notafiscal.rateio.itemnotafiscalrateio.ItemNotaFiscalRateioDTO">
								<tr>
									<td>
										<html:select styleClass="form-control input-sm" styleId="centroCusto${i}" property="notaFiscalRateio.itens[${i}].centroCusto.id" name="notaFiscalRateioReceitaForm">
											<html:option value="">Selecione...</html:option>
											<html:optionsCollection name="notaFiscalRateioReceitaForm" property="comboCentroCusto" label="label" value="value" />
										</html:select>								
									</td>
									...
									<logic:equal value="Cultura" name="itemCorrente" property="tipo">
										<td class="text-center" style="background-color: #e6e6e6;" colspan="2">
											<strong>${itemCorrente.tipo}:</strong>
											${itemCorrente.cultura.nomeCompleto} - ${itemCorrente.safra.nomeCompleto}
										</td>
									</logic:equal>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var valor = '${notaFiscalRateioReceitaForm.notaFiscalRateio.valorTotal}';		
		if (valor == null || valor == '') {
			valor = '0.0';
		}	
		valor = parseFloat(valor.replace('.', '').replace(',', '.').replace('R$', ''), 10);
		$('#valorTotalNotaFiscalRateioReceita').text(formatarDinheiro((valor).toFixed(2)));
