...
public class NotaFiscalRateioReceitaForm extends AbstractCRUDForm {
	...
	private TituloReceberDTO parcelaSelecionada;

	public TituloReceberDTO getParcelaSelecionada() {
		if ( this.parcelaSelecionada == null ) {
			this.parcelaSelecionada = new TituloReceberDTO();
		}
		return this.parcelaSelecionada;
	}

	public void setParcelaSelecionada( TituloReceberDTO parcelaSelecionada ) {
	...
	public ArrayList< LabelValueBean > getComboFormaPagamento( String isADM ) throws Throwable {
		...
		try {
			List< FormaPagamentoDTO > encontrados = FinanceiroFacade.getInstance().getComboFormaPagamento( String.valueOf( isADM ), null, getNotaFiscalRateio().getContaReceber().getTipo(),
				FormaPagamentoDTO.TIPO_RECEITA );
	...
	public ArrayList< LabelValueBean > getComboFormaPagamentoAVista( String isADM ) throws Throwable {
		...
		try {
			List< FormaPagamentoDTO > encontrados = FinanceiroFacade.getInstance().getComboFormaPagamentoAVista( String.valueOf( isADM ), null, FormaPagamentoDTO.TIPO_RECEITA );
	...
	public void limparCamposCadastro( HttpServletRequest request ) {
		...
		this.getNotaFiscalRateio().getContaReceber().setData( DateUtil.parseLocalDate( LocalDate.now() ) );
		this.getNotaFiscalRateio().getContaReceber().setSituacao( SituacaoFinanceiroEnum.ABERTA.getValor() );
		getNotaFiscalRateio().setTipo( NotaFiscalRateioEnum.TIPO_RECEITA.getValor() );
