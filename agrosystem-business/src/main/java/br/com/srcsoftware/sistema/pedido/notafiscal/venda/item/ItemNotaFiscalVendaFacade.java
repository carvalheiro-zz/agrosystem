package br.com.srcsoftware.sistema.pedido.notafiscal.venda.item;

import java.util.HashMap;
import java.util.List;

import br.com.srcsoftware.modular.manager.exceptions.ApplicationException;
import br.com.srcsoftware.modular.manager.paginacao.Paginacao;
import br.com.srcsoftware.sistema.pedido.notafiscal.venda.ItemNotaFiscalVendaDTO;

public class ItemNotaFiscalVendaFacade implements ItemNotaFiscalVendaServiceInterface{

	private final ItemNotaFiscalVendaServiceImpl service;

	private ItemNotaFiscalVendaFacade(){
		this.service = new ItemNotaFiscalVendaServiceImpl();
	}

	public static ItemNotaFiscalVendaFacade getInstance() {
		return new ItemNotaFiscalVendaFacade();
	}

	@Override
	public List< ItemNotaFiscalVendaDTO > filtrarPorDataNotaFiscalVendaBetween( String dataInicial, String dataFinal ) throws ApplicationException {
		return service.filtrarPorDataNotaFiscalVendaBetween( dataInicial, dataFinal );
	}

	@Override
	public List< ItemNotaFiscalVendaDTO > filtrar( Paginacao paginacao, HashMap< String, String > camposOrders, ItemNotaFiscalVendaDTO dto ) throws ApplicationException {
		return service.filtrar( paginacao, camposOrders, dto );
	}

}
