...
public final class ItemPedidoDAOImpl implements ItemPedidoDAOInterface{

	@Override
	@SuppressWarnings( "unchecked" )
	public ArrayList< ItemPedidoPO > filtrarPorDataPedidoBetween( LocalDate dataInicial, LocalDate dataFinal ) throws ApplicationException {
		...
		try {
			...
			HQL.append( " SELECT itemPedido.* FROM " );
			HQL.append( Constantes.SCHEMA + ".itens_pedidos itemPedido, " );
			HQL.append( Constantes.SCHEMA + ".pedidos pedido, " );
			HQL.append( " WHERE itemPedido.idPedido = pedido.id " );
			HQL.append( " and itemPedido.data between :dataInicialParam and :dataFinalParam" );
			...
			ArrayList< ItemPedidoPO > encontrados = (ArrayList< ItemPedidoPO >) query.getResultList();
	...
	@SuppressWarnings( "unchecked" )
	@Override
	public List< ItemPedidoPO > filtrar( Paginacao paginacao, HashMap< String, String > camposOrders, ItemPedidoPO poFilter ) throws ApplicationException {
		...
		try {
			...
			CriteriaQuery< ItemPedidoPO > criteria = builder.createQuery( ItemPedidoPO.class );

			// DEFININDO O OBJETO BASE DA SELECT
			Root< ItemPedidoPO > root = criteria.from( ItemPedidoPO.class );

			// Definindo os parametros do WHERE
			Predicate[ ] parametros = this.montarParametros( builder, root, poFilter );

			// Executando A Consulta
			List< ItemPedidoPO > encontrados = hibernate.filtrar( paginacao, camposOrders, builder, root, criteria, parametros, ItemPedidoPO.class );
	...
	private Predicate[ ] montarParametros( CriteriaBuilder builder, Root< ItemPedidoPO > root, ItemPedidoPO poFilter ) {
