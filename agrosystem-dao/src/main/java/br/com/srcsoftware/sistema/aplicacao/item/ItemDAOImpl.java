...
public class ItemDAOImpl implements ItemDAOInterface{
	...
	@Override
	public ItemPO inserir( HibernateConnection hibernate, ItemPO po ) throws ApplicationException {
		try {
			return (ItemPO) hibernate.inserir( po );
	...
	@Override
	public void alterar( HibernateConnection hibernate, ItemPO po ) throws ApplicationException {
		try {
	...
	@Override
	public void excluir( HibernateConnection hibernate, ItemPO po ) throws ApplicationException {
		try {
	...
	@Override
	public ItemPO filtrarPorId( Long id ) throws ApplicationException {
		try {
			return (ItemPO) new HibernateConnection().filtrarPorId( ItemPO.class, id );
		} catch ( ApplicationException e ) {
			throw e;
		} catch ( Exception e ) {
			throw new ApplicationException( "Erro inesperado" + System.lineSeparator() + e.getMessage().trim(), e );
	...
	@Override
	public List< ItemPO > filtrar( Paginacao paginacao, HashMap< String, String > camposOrders, HashMap< String, ArrayList< Object > > camposBetween, ItemPO poFilter, String tipoManutencao ) throws ApplicationException {
		...
		try {
			...
			CriteriaQuery< ItemPO > criteria = builder.createQuery( ItemPO.class );

			// DEFININDO O OBJETO BASE DA SELECT
			Root< ItemPO > root = criteria.from( ItemPO.class );
			...
			List< ItemPO > encontrados = hibernate.filtrar( paginacao, camposOrders, builder, root, criteria, parametros, ItemPO.class );
	...
	private Predicate[ ] montarParametros( CriteriaBuilder builder, Root< ItemPO > root, ItemPO poFilter, HashMap< String, ArrayList< Object > > camposBetween, String tipoManutencao ) {
		...
		if ( !Utilidades.isNuloOuVazio( poFilter.getId() ) ) {
			...
			if ( !Utilidades.isNuloOuVazio( poFilter.getAlmoxarife() ) ) {
				...
				Join< ItemPO, FuncionarioPO > almoxarifeJoin = null;
			...
			if ( !Utilidades.isNuloOuVazio( poFilter.getRequerente() ) ) {
				...
				Join< ItemPO, FuncionarioPO > requerenteJoin = null;
			...
			if ( !Utilidades.isNuloOuVazio( poFilter.getProduto() ) ) {
				...
				Join< ItemPO, ProdutoPO > produtoJoin = null;
			...
			if ( !Utilidades.isNuloOuVazio( poFilter.getSetor() ) ) {
				...
				Join< ItemPO, SetorPO > setorJoin = null;
			...
			if ( !Utilidades.isNuloOuVazio( poFilter.getSafra() ) ) {
				...
				Join< ItemPO, SafraPO > safraJoin = null;
			...
			if ( !Utilidades.isNuloOuVazio( poFilter.getCultura() ) ) {
				...
				Join< ItemPO, CulturaPO > culturaJoin = null;
			...
			if ( !Utilidades.isNuloOuVazio( poFilter.getVeiculo() ) ) {
				...
				Join< ItemPO, VeiculoPO > veiculoJoin = null;
			...
			if ( !Utilidades.isNuloOuVazio( poFilter.getImplemento() ) ) {
				...
				Join< ItemPO, ImplementoPO > implementoJoin = null;
			...
			if ( !Utilidades.isNuloOuVazio( poFilter.getImovel() ) ) {
				...
				Join< ItemPO, ImovelPO > imovelJoin = null;
