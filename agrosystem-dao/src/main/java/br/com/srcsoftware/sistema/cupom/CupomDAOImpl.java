...
public class CupomDAOImpl implements CupomDAOInterface{

	@Override
	public CupomPO inserir( HibernateConnection hibernate, CupomPO po ) throws ApplicationException {
		try {
			return (CupomPO) hibernate.inserir( po );
	...
	@Override
	public void alterar( HibernateConnection hibernate, CupomPO po ) throws ApplicationException {
		try {
	...
	@Override
	public void excluir( HibernateConnection hibernate, CupomPO po ) throws ApplicationException {
		try {
	...
	@Override
	public CupomPO filtrarPorId( Long id ) throws ApplicationException {
		try {
			return (CupomPO) new HibernateConnection().filtrarPorId( CupomPO.class, id );
		} catch ( ApplicationException e ) {
			throw e;
		} catch ( Exception e ) {
			throw new ApplicationException( "Erro inesperado" + System.lineSeparator() + e.getMessage().trim(), e );
	...
	@Override
	public List< CupomPO > filtrar( Paginacao paginacao, HashMap< String, String > camposOrders, HashMap< String, ArrayList< Object > > camposBetween, CupomPO poFilter ) throws ApplicationException {
		...
		try {
			...
			CriteriaQuery< CupomPO > criteria = builder.createQuery( CupomPO.class );

			// DEFININDO O OBJETO BASE DA SELECT
			Root< CupomPO > root = criteria.from( CupomPO.class );

			// Definindo os parametros do WHERE
			Predicate[ ] parametros = this.montarParametros( builder, root, poFilter, camposBetween );

			// Executando A Consulta
			List< CupomPO > encontrados = hibernate.filtrar( paginacao, camposOrders, builder, root, criteria, parametros, CupomPO.class );
	...
	private Predicate[ ] montarParametros( CriteriaBuilder builder, Root< CupomPO > root, CupomPO poFilter, HashMap< String, ArrayList< Object > > camposBetween ) {
		...
		if ( !Utilidades.isNuloOuVazio( poFilter.getId() ) ) {
			...
			if ( !Utilidades.isNuloOuVazio( poFilter.getNotaEmitida() ) ) {
				lista.add( builder.equal( root.get( "notaEmitida" ), poFilter.getNotaEmitida() ) );
			}
			...
			if ( !Utilidades.isNuloOuVazio( poFilter.getFornecedor() ) ) {
				...
				Join< CupomPO, FornecedorJuridicoPO > fornecedorJoin = null;
			...
			if ( !Utilidades.isNuloOuVazio( poFilter.getItens() ) ) {
				Join< CupomPO, ItemCupomPO > itemJoinList = null;

				ItemCupomPO itemFiltrar = poFilter.getItens().iterator().next();
